...
import type {
  ...
  AutonomyLevel, AegisProposal, CommStatus, UplinkCommand, UplinkResult,
} from '@/types';
...
export const simulationApi = {
  ...
  setAutonomy: (roverId: string, level: AutonomyLevel) =>
    http.patch<UplinkResult>(`/simulation/rovers/${roverId}/autonomy`, { level }).then(r => r.data),
  ...
  approveAegisProposal: (roverId: string, missionId: string) =>
    http.post<UplinkResult>(`/simulation/rovers/${roverId}/aegis-proposal/approve`, null, {
      params: { mission_id: missionId },
    }).then(r => r.data),
  rejectAegisProposal: (roverId: string) =>
    http.post<UplinkResult>(`/simulation/rovers/${roverId}/aegis-proposal/reject`).then(r => r.data),
};
...
export const telemetryApi = {
  ...
  resolveAnomaly: (anomalyId: string, resolution = 'ignored') =>
    http.patch<UplinkResult>(`/telemetry/anomalies/${anomalyId}/resolve`, { resolution }).then(r => r.data),
  dismissAll: (missionId: string) =>
    http.post<{ delivered: Anomaly[]; queued: UplinkCommand[] }>(`/telemetry/anomalies/${missionId}/dismiss-all`).then(r => r.data),
};

// ─── Communication Windows ───────────────────────────────────────────────────

export const commApi = {
  status: () => http.get<CommStatus>('/comm/status').then(r => r.data),
  queue: (missionId: string) => http.get<UplinkCommand[]>(`/comm/${missionId}/queue`).then(r => r.data),
};
